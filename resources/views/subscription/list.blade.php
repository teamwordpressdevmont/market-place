                    ...
                    <tbody>
                        ...
                        @foreach($subscriptions as $index => $subscription)
                            <tr class="bg-white border-b border-gray-200">
                                    ...
                                    {{ ($subscriptions->currentPage() - 1) * $subscriptions->perPage() + $index + 1 }}
                                </th>
                                <th scope="row" class="px-6 py-5 whitespace-nowrap text-xs text-mat font-bold">
                                    {{ $subscription->id }}
                                </th>
                                ...
                                <td class="px-6 py-5 whitespace-nowrap text-xs text-mat font-normal">
                                    <div class="site_user_dropdown">
                                        <div class="cursor-pointer w-[30px] ml-auto bg-[#eee] px-1 py-2 rounded-md flex justify-center items-center hover:bg-gray-300 transition" data-dropdown-toggle="userDropdown-action-{{ $subscription->id }}" data-dropdown-placement="bottom-end">
                                        ...
                                        <div id="userDropdown-action-{{ $subscription->id }}" class="z-10 bg-white divide-y rounded-xl w-[122px] border border-[#d3d3d3] hidden" aria-hidden="true" data-popper-placement="bottom-end">
                                            <ul class="bg-white text-sm rounded-xl overflow-hidden">
                                                <li class="border-b border-[#d3d3d3]">
                                                    <a href="{{ route('subscription.edit', $subscription->id) }}" class="text-left block px-3 py-3 text-xs font-light transition hover:bg-[#222222] hover:text-white text-[#222222]">Edit</a>
                                                </li>
                                                <li class="">
                                                    <a href="{{ route('subscription.delete', $subscription->id) }}" class="text-left block px-3 py-3 text-xs font-light transition hover:bg-[#222222] hover:text-white text-[#222222]">Delete</a>
                                                </li>
                                            </ul>
                                        </div>
                                    </div>
