    ...
    <div class="mt-5 bgShadow">
        <div class="pt-10 pb-8">
            <div class="grid lg:grid-cols-2 grid-cols-1 items-start mb-8 lg:gap-0 gap-4">
                <div>
                    <h1 class="font-semibold lg:text-4xl md:text-2xl text-xl mb-2 text-mat">Customer Service plans</h1>
                    ...
                    <thead class="text-xs text-gray-700 bg-[#eee] border-b border-[#22222233]">
                        <tr>
                            ...
                            <th scope="col" class="px-6 py-3 text-[#ABABAB] font-[500]">
                                <a href="{{ route('customer.list', array_merge(request()->all(), ['sort_by' => 'title', 'sort_direction' => request('sort_direction') == 'asc' ? 'desc' : 'asc'])) }}">
                                    Title
                                </a>
                            </th>
                    ...
                    <tbody>
                        ...
                        @foreach($customers as $index => $customer)
                            <tr class="bg-white border-b border-gray-200">
                                ...
                                <td class="px-6 py-5 whitespace-nowrap text-xs text-mat font-normal">{{ $customer->title }}</td>
                                <td class="px-6 py-5 whitespace-nowrap text-xs text-mat font-normal">{!! html_entity_decode($customer->description) !!}</td>
                                <td class="px-6 py-5 whitespace-nowrap text-xs text-mat font-normal">{{ $customer->price }}</td>
                                <td class="px-6 py-5 whitespace-nowrap text-xs text-mat font-normal">
                                    <div class="site_user_dropdown">
                                        <div class="cursor-pointer w-[30px] ml-auto bg-[#eee] px-1 py-2 rounded-md flex justify-center items-center hover:bg-gray-300 transition" data-dropdown-toggle="userDropdown-action-{{ $customer->id }}" data-dropdown-placement="bottom-end">
                                        ...
                                        <div id="userDropdown-action-{{ $customer->id }}" class="z-10 bg-white divide-y rounded-xl w-[122px] border border-[#d3d3d3] hidden" aria-hidden="true" data-popper-placement="bottom-end">
                                            <ul class="bg-white text-sm rounded-xl overflow-hidden">
                                                <li class="border-b border-[#d3d3d3]">
                                                    <a href="{{ route('customer.edit', $customer->id) }}" class="text-left block px-3 py-3 text-xs font-light transition hover:bg-[#222222] hover:text-white text-[#222222]">Edit</a>
                                                </li>
                                                <li class="">
                                                    <a href="{{ route('customer.delete', $customer->id) }}" class="text-left block px-3 py-3 text-xs font-light transition hover:bg-[#222222] hover:text-white text-[#222222]">Delete</a>
                                                </li>
                                            </ul>
                                        </div>
                                    </div>
