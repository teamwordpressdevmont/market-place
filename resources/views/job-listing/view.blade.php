   ...
   <div class="">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-5">
         ...
         <div class="lg:col-span-2 p-4">
            ...
            <h4 class="font-semibold lg:text-2xl mt-4 mb-2 flex gap-2">{{ $OrderDetail->order->tradeperson->first_name }} {{ $OrderDetail->order->tradeperson->last_name }}<svg xmlns="http://www.w3.org/2000/svg" width="28" height="27" viewBox="0 0 28 27" fill="none">
            ...
            <p class="text-[#ABABAB] text-xs mb-3 flex leading-none">    {{ $OrderDetail->order->tradeperson->categories->pluck('name')->join(', ') }}
            </p>
            <p class="text-[#2B2B2B] text-sm mt-2 flex leading-[1.1rem] mb-5">{{ $OrderDetail->order->tradeperson->about_me }}</p>
            ...
            <div class="flex gap-4">
               @if($OrderDetail->order->tradeperson->certificate)
                   @foreach(explode(',', $OrderDetail->order->tradeperson->certificate) as $certificate)
                       <img src="{{ asset('/certificates/' . trim($certificate)) }}" class="h-auto max-w-full">
                   @endforeach
               @else
